    ...
    <div class="content container-fluid">
        ...
        <div class="row g-3 text-capitalize">
            <!-- Earnings (Monthly) Card Example -->
             @foreach ($get_qr as $qr)
            <div class="col-md-3">
                <div class="col-lg-12 col-md-9">
                    <a class="form-alert" href="javascript:" data-id="food-{{$qr->id}}"
                        data-message="{{ translate('Want to active this qr-scanner ?') }}"
                        title="{{ translate('messages.Change_QR_Scanner') }}">
                        <img 
                        {{-- C:\xampp\htdocs\my\restro\storage\app\public\qrcodes --}}
                        @if ($qr->qr_scanner)
                        src="{{asset('storage/app/public/qrcodes')}}/{{$qr->qr_scanner}}" 
                        @else
                        src="{{ asset('public/assets/admin/img/100x100/2.png') }}"
                        @endif
                         alt="img">
                    </a>
                    <form action="{{route('vendor.business-settings.change_status',[$qr->id])}}"
                            method="post" id="food-{{$qr->id}}">
                        @csrf @method('patch')
                    </form>
            ...
            {{-- <div class="col-md-3">
                <div class="col-lg-12 col-md-9">
                    <img src="{{ asset('public/assets/admin/img/100x100/2.png') }}" alt="img">
                    <h5 class=" text-center cash--subtitle text-white">
                        {{ translate('messages.activated') }}
                    </h5>
                </div>
            </div>
            <div class="col-md-3">
